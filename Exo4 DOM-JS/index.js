...
chanson.paroles.forEach(element =>{
  ...
  paragraphAjout.classList = element.type;
  parolesTest.appendChild(paragraphAjout)
  paragraphAjout.innerHTML = element.contenu;
  const lignes = element.contenu.map(ligne => ligne + "<br>").join("");
  paragraphAjout.innerHTML = lignes;
})
...
//
//
let refrain = document.querySelectorAll("p");
refrain.forEach(function(selection) {
    selection.innerHTML = "";
    chanson.paroles.forEach(function(element) {
    //   const span = document.createElement('span');
    //   span.classList = 'contenu';
    //   const hidden = document.createElement('span');
    //   hidden.classList = element.type;
    //          selection.append(span)
          // if (paroles.type === 'choeur') {
          //     hidden.classList = 'choeur hidden';
          //     hidden.innerHTML = "[Choeur]<br>";
          // } else {
          //     hidden.classList = paroles.type;
          // }
          // const lignes = paroles.contenu.map(ligne => ligne + "<br>").join("");
          // parolesTest.appendChild(selection);
          // selection.append(span);
          // selection.prepend(hidden);
          // span.innerHTML = lignes;
